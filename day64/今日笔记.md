...
DateField 可以直接使用 register_time__month register_time__year register_time__day
TimeField 可以直接使用  register_time__hour  register_time__minute register_time__second

DateTimeField 不可以直接使用 , 
查询year month day 要添加__date
register_time__date__month register_time__date__year register_time__date__day
查询hour minute second要添加__time
register_time__time__second__gt  register_time__time__minute__gt register_time__time__hour__gt

非常有规律呀  Date 对应年月日  time对应时分秒


register_time__date__month  register_time__date__month用法后面验证是错的
其实只要数据库添加一下时区就可以用正常的的
    # res = models.User.objects.filter(register_time__month='1')
    # res = models.User.objects.filter(register_time__hour='17')
查询
